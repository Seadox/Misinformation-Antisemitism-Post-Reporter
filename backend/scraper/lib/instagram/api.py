...
class InstagramScraper(ScraperAbs):
    ...
    def init_client(self, config: dict = {}):
        ...
        user_settings = config.get(
            "filename", f"{self.__PATH}/{username}_settings.json")
        ...
        try:
            if not os.path.isfile(user_settings):
                ...
                print(
                    f'instagram_scraper -- init_client() -- Reusing settings: {0!s}'.format(user_settings))
        ...
        try:
            for hash_tag in keywords:
                ...
                print("%s_scraper -- get_text_messages_by_keywords() -- found %s posts for hash_tag \"%s\"" %
                      (self._source, str(len(response["sections"])), str(hash_tag)))
                ...
                for post in response["sections"]:
                    ...
                    if post["feed_type"] == "channel":
                        ...
                        print("%s -- get_text_messages_by_keywords() -- skipping unsupported media_type" %
                              (self._source, str(post["feed_type"])))
                        continue
            ...
            print("%s_scraper -- get_text_messages_by_keywords() -- unexpected execution -- %s" %
                  (self._source, str(e)))
            traceback.print_exc()
